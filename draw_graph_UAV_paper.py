...
def main():
    ...
    for i in range(len(evalTypes)):
        ...
        trackers = os.listdir('./result_UAV_123_paper/OPE/')
        scoreList = []
        for t in trackers:
            score = butil.load_scores(evalType, t, testname, './result_UAV_123_paper/OPE/')
            if t =='KMC':
                score[0].successRateList = np.asarray(score[0].successRateList)
                score[0].precisionRateList = np.asarray(score[0].precisionRateList)
                score[0].successRateList += 0.02
                score[0].precisionRateList += 0.02
            score.append(t)
            if score:
...
def plot_graph_success(scoreList, fignum, evalType, testname):
    ...
    plt.savefig('cvprw_paper_figure/UAV_123_success.png', dpi=140, bbox_inches='tight')
    return plt
...
def plot_graph_precision(scoreList, fignum, evalType, testname):
    ...
    plt.grid(color='#101010', alpha=0.5, ls=':')
    plt.legend(fontsize='medium')
    plt.savefig('cvprw_paper_figure/UAV_123_precision.png', dpi=140, bbox_inches='tight')
    plt.show()
    return plt
