...
UAV_result_dir = '/home/stevenwudi/Documents/Python_Project/UAV/results_OPE_UAV123'
...
for seq in seqs:
    ...
    for tracker_name in Tracker_names:
        ...
        if idx>-1:
            ...
            dict = {}
            dict['seqName'] = seq[:idx-1]
            dict['tracker'] = tracker_name
            dict['evalType'] = "OPE"
            dict['resType'] = "rect"
            if mat['results']['res'].shape[1] == 4:
                dict['res'] = mat['results']['res'].tolist()
                if not os.path.exists(os.path.join('./../results_UAV123/OPE', tracker_name)):
                    os.mkdir(os.path.join('./../results_UAV123/OPE', tracker_name))
                with open(os.path.join('./../results_UAV123/OPE',tracker_name, dict['seqName'] + '.json'), 'w') as outfile:
