...
def dense_gauss_kernel(sigma, x, y=None):
    ...
    if use_hog:
        row_shift, col_shift, dummy = pylab.floor(pylab.array(x.shape) / 2).astype(int)
    else:
    ...
    if use_hog:
        xy = pylab.real(np.sum(pylab.real(xy_complex), axis=2))
    else:
...
def track(input_video_path):
    ...
    if use_hog:
        sz_y = pylab.floor(target_sz * (1 + padding) / cell_sz)
        # desired output (gaussian shaped), bandwidth proportional to target size
        output_sigma = pylab.sqrt(pylab.prod(target_sz)) * output_sigma_factor / cell_sz
    else:
        sz_y = pylab.floor(target_sz * (1 + padding))
        output_sigma = pylab.sqrt(pylab.prod(target_sz)) * output_sigma_factor

    grid_y = pylab.arange(sz_y[0]) - pylab.floor(sz_y[0]/2)
    grid_x = pylab.arange(sz_y[1]) - pylab.floor(sz_y[1]/2)
    ...
    for frame, image_filename in enumerate(img_files):
        ...
        if use_hog:
            x, hog_image = hog(x, orientations=bin_num, pixels_per_cell=(cell_sz, cell_sz), cells_per_block=(1, 1), visualise=True)
        ...
        if not is_first_frame:
            ...
            if use_hog:
                row, col = [(row + 1 + 1/2) * cell_sz, (col + 1+1/2) * cell_sz]
...
def main():
    video_path = r'D:\Tracking\circulant_matrix_tracker\tiger2'
    track(video_path)
